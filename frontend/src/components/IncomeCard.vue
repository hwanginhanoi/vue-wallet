...
const fetchData = async () => {
    try {
        const response = await axios.get('http://localhost:3000/index/income', {
            params: { userId: userStore.user.id }
        });
        const { last7DaysIncome: last7DaysIncome , percentageChange: changePercentage } = response.data;
        todayRemaining.value = last7DaysIncome;
...
<template>
    <div class="flex flex-col p-6 bg-white border border-gray-200 rounded-lg shadow">
        <a href="#">
            <h5 class="mb-2 text-2xl font-semibold tracking-tight text-gray-900">Income</h5>
        </a>
